...
define(['config', 'logger', 'backbone', 'underscore'], function (config, Logger, Backbone, _) {
    ...
    function createInstance () {
        ...
        _.each(dispatcher, function (events, category) {
            ...
            var categoryEvents = {
                ...
                off: function (callback, _this) {
                    if(!callback && _this){
                        ...
                        var cat = dispatcher[category];
                        _.each(cat, function(v, k){
                            k != 'off' && k != 'all' && cat[k].off(callback, _this);
                        });
                    }
                }
            };
            ...
            _.each(events, function (value, event) {
                ...
                // e.g.: dispatcher.global.onSomething(this, fn);
                ...
                // trigger listener
                // e.g.: dispatcher.global.onSomething.trigger();
